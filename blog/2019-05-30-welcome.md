...
Importantly, it provides this in a way that the genomics epi can focus on genomics epi problems without being concerned about the underlying infrastructure or the bioinformatics tooling. Although, that information is available if needed. In many respects, the genomics epi person wants to get places but they don't want to know how an internal combustion engine works or they may even not want to learn how to drive. AusTrakka aims to be like a car sharing app (without the social issues involved). The genomics epi just needs to punch in their destination, and AusTrakka takes care of the rest. 
